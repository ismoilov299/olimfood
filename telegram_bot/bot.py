...
USER_MSGS = {
    "confirmed":  "✅ Буюртмангиз <b>#{id}</b> тасдиқланди! Тез орада тайёрланади 🍽️",
    "preparing":  "👨‍🍳 Буюртмангиз <b>#{id}</b> тайёрланмоқда! Кутиб туринг...",
    "delivering": "🚗 Буюртмангиз <b>#{id}</b> йўлда! Эшикни кутинг 🏠",
    "delivered":  "🟢 Буюртмангиз <b>#{id}</b> етказилди! Иштаҳангиз очиқ бўлсин! 🙏",
    "cancelled":  "❌ Буюртмангиз <b>#{id}</b> бекор қилинди. Кечирасиз.",
}

PAY_LABELS = {"naqd": "💵 Нақд пул"}
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Show language selection only the first time; afterwards jump straight to the welcome screen."""
    if update.message:
        msg = update.message
    else:
    ...
    if ctx.user_data.get("lang"):
        ...
        await _send_welcome(msg, ctx, edit=False)
        return

    await msg.reply_text(_LANG_PROMPT, reply_markup=_LANG_KB)
...
def admin_kb(oid):
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("✅ Тасдиқлаш",        callback_data=f"as_confirmed_{oid}"),
            InlineKeyboardButton("👨‍🍳 Тайёрланмоқда",   callback_data=f"as_preparing_{oid}"),
        ],
        [
            InlineKeyboardButton("🚗 Етказилмоқда",    callback_data=f"as_delivering_{oid}"),
            InlineKeyboardButton("🟢 Етказилди",       callback_data=f"as_delivered_{oid}"),
        ],
        [InlineKeyboardButton("❌ Бекор қилиш",         callback_data=f"as_cancelled_{oid}")],
    ])


def _feedback_kb(order_id: int) -> "InlineKeyboardMarkup | None":
...
async def finish_order(update: Update, ctx: ContextTypes.DEFAULT_TYPE, msg):
    ...
    await msg.reply_text(
        ...
        f"📦 Буюртма <b>#{order_id}</b>\n"
        f"💰 Жами: <b>{fmt(total)} сўм</b>\n"
        f"💳 Тўлов: {pay_disp}\n\n"
    ...
    if ADMIN_CHAT:
        items_lines = "\n".join(
            ...
            f"{h(it['name'])} × {it['qty']} = {fmt(it['price']*it['qty'])} сўм"
            for it in items
        )
        note_line = f"💬 {h(note)}\n" if note else ""
        admin_txt = (
            ...
            f"💳 {pay_disp}\n\n"
            f"<b>Маҳсулотлар:</b>\n{items_lines}\n\n"
            f"📦 Етказиш: {fmt(DELIVERY)} сўм\n"
            f"💰 <b>Жами: {fmt(total)} сўм</b>"
        )
        try:
...
async def cancel_checkout(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if q:
        await q.answer()
        await q.message.reply_text("❌ Буюртма бекор қилинди.")
    elif update.message:
        await update.message.reply_text("❌ Буюртма бекор қилинди.")
    # Return to main menu
    ctx.user_data.setdefault("cart", [])
    await _send_welcome(
...
def main():
    ...
    print("=" * 55)
    print("Olimbrand Bot ishga tushdi!")
